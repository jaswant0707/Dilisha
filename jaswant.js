...
let blocks = document.getElementsByClassName("blockk"),
    block = blocks[0],
    love = document.getElementsByClassName("lovee")[0],
    timer = null,
    index = 0, 
    clone_block;  
...
const observer = new IntersectionObserver((entries, observer) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            console.log("Class .k is now visible! Starting animation...");
            ...
            observer.unobserve(entry.target); // Stop observing after 1st trigger
        }
    });
}, {
...
const dElement = document.querySelector('.k');
if (dElement) {
    observer.observe(dElement);
}
